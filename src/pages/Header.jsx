...
function MobileHeader() {
  const { scrollY } = useScroll(); // To track scroll position
  const [hidden, setHidden] = useState(false); //for navbar to hide on scroll
  ...
  const [isMobileMenuVisible, setMobileMenuVisible] = useState(false); // State to manage mobile menu visibility

  const [isDesktopMenuHovered, setIsDesktopMenuHovered] = useState(false); // State to manage desktop menu hover status
  ...
  return (
    <>
      {/* <SearchBar /> */}
      <div className="static w-full min-w-[320px]  z-50">
        <header className="sticky top-0 flex flex-col justify-center items-center h-auto z-50">
          <div className="z-10 h-9  overflow-y-hidden flex items-center bg-[#d2bab6] w-full">
            <Marquee
              pauseOnHover
              autoFill
              gradient
          ...
          <motion.div
            variants={variants}
            // animate={hidden ? "showing" : "notShowing"}
